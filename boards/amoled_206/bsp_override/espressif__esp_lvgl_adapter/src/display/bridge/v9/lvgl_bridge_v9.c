                                            ...
                                            uint8_t *color_map)
{
    ...
    const bool first_flush = !impl->te_frame_in_progress;
    const bool last_flush = lv_display_flush_is_last(disp);

    if (first_flush) {
        impl->te_frame_in_progress = true;
        size_t frame_transfer_size = last_flush ? (size_t)lv_area_get_size(area) * color_bytes :
                                     (size_t)bridge_h_res(impl) * bridge_v_res(impl) * color_bytes;

        if (impl->cfg.te_ctx) {
            esp_lv_adapter_te_sync_begin_frame(impl->cfg.te_ctx, frame_transfer_size);
    ...
    ulTaskNotifyValueClear(NULL, ULONG_MAX);
    esp_err_t ret = esp_lcd_panel_draw_bitmap(panel_handle, offsetx1, offsety1,
    ...
    if (last_flush || ret != ESP_OK) {
        if (impl->cfg.te_ctx && impl->te_frame_in_progress) {
            esp_lv_adapter_te_sync_record_tx_done(impl->cfg.te_ctx);
        }
        impl->te_frame_in_progress = false;
    }
