...
esp_err_t bsp_display_brightness_set(int brightness_percent)
{
    ...
    esp_err_t ret = esp_lcd_panel_co5300_set_brightness(panel_handle, brightness_percent);
    if (ret == ESP_OK)
    {
...
{
    assert(cfg != NULL);
    uint16_t buffer_height = cfg->buffer_height ? cfg->buffer_height : LVGL_BUFFER_HEIGHT;
...
lv_display_t *bsp_display_start(void)
{
    bsp_display_cfg_t cfg = {
        ...
        .buffer_height = LVGL_BUFFER_HEIGHT,
        .use_psram = false,
    };
...
{
    return esp_lv_adapter_lock(timeout_ms == UINT32_MAX ? -1 : (int32_t)timeout_ms);
}
